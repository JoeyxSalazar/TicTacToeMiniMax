...
if __name__ == '__main__':
    ...
    s = TicTacToeState()
    s.player = Square.X
    if (temp == 1): #Human is always 'O'
        s.playerToMove = Square.O
        s.player = Square.O
    else :          #Computer is always 'X'
        s.playerToMove = Square.X
    while (True):
        if (s.playerToMove == Square.X):
            minimax=MiniMax() #Computer performs mini-max
            s = s.getResult(minimax.MinimaxDecision(s, mark))
        else :
            print("Which square do you want to set? (1--9) ")#Else human makes a decision
            while (True):
                temp = (int)(input())
                if  temp >= 1 & temp <= 9 :
                     break
